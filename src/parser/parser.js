...
export default function() {
  ...
  let actions = {
    ...
    keep: function(kept) {
      if (start == -1) {
        ...
        if (
          charsInFront.trim() == ";" &&
          input.substring(posInFront - 5, posInFront + 1) == "&nbsp;"
        ) {
          posInFront -= 6;
          ...
          if (!charsInFront.trim().length) {
            ...
            posInFront -= 1;
            charsInFront =
              input.substring(posInFront, posInFront + 1) + charsInFront;
          }
        }
        ...
        if (
          charsInFront.length &&
          !new RegExp(whitespace + "|" + nextline + "|[(,'\"]|[^\\d+]").test(
            charsInFront
          )
        ) {
          if (charsInFront.slice(-1) == ";") {
            ...
            if (
              input
                .substring(posInFront - 2, posInFront + 1)
                .toLowerCase()
                .trim() == "f;" ||
              input
                .substring(posInFront - 4, posInFront + 1)
                .toLowerCase()
                .trim() == "fax;"
            ) {
              ignore = true;
            ...
            if (
              ...
              new RegExp("\\d+").test(
                input.substring(posInFront - 2, posInFront)
              )
            ) {
              ignore = true;
            ...
            if (
              input
                .substring(posInFront - 1, posInFront + 1)
                .trim()
                .toLowerCase() == "f" ||
              input
                .substring(posInFront - 3, posInFront + 1)
                .trim()
                .toLowerCase() == "fax"
            ) {
              ignore = true;
          ...
          if (kept == "0") {
            ...
            if (
              input.substring(posInFront - 1, posInFront + 1).slice(-1) ==
                "(" &&
              ...
              if (
                input.substring(posInFront + 1, posInFront + 2) != " " &&
                input.substring(posInFront + 1, posInFront + 7) != "&nbsp;"
              ) {
                if (
                  new RegExp("[a-zA-Z]").test(
                    input.substring(posInFront, posInFront + 2)
                  )
                ) {
                  ignore = true;
        ...
        if (!ignore) {
          ...
          if (
            charsInFront.length &&
            !new RegExp(whitespace + "|" + nextline + "|[(,'\"]").test(
              charsInFront
            )
          ) {
            if (charsInFront.slice(-1) == ";") {
              ...
              if (
                ...
                if (debug)
                  console.log(
                    "    > ignore",
                    '"' + input.substring(posInFront - 10, posInFront + 1) + '"'
                  );
              } else {
                if (
                  BICCODES.includes(
      ...
      if (new RegExp(rules[12].pattern).test(kept)) {
        ...
        kept = "-";
        if (debug)
          console.error("force replace matched dash with a standard dash");
      }

      result += kept;
      buffer = "";
      if (debug)
        console.log("keep", '"' + kept + '"', "->", '"' + result + '"');
  ...
  rules[13] = {
    pattern: "^\\b|" + whitespace + nextline + "|" + nbsp + "|$",
    action: actions.end,
    length: 6
  };
  ...
  let parse = function(text) {
    ...
    if (debug)
      console.warn(
        "check whatsever is in the buffer (" + buffer + ")",
        input.length,
        pos,
        peekpos
      );
  ...
  let validate = function() {
    ...
    if (new RegExp("[^\\+\\d\\(\\)\\-(\\xA0\\x20\\t)]").test(just_text)) {
      if (debug)
        console.log(
          'invalid: unwanted characters in raw result: "' + just_text + '"'
        );
      return false;
    }
    ...
    if (new RegExp("^00|[^89]00").test(just_digits.substring(0, 3))) {
      if (debug)
        console.log("invalid: impossible area code after ((+|00)31|0)");
      return false;
    }
    ...
    if (just_digits.substring(0, 1) == "8") {
      if (just_digits.substring(0, 3) == "800") {
        if (debug)
          console.log(
            "invalid: incorrect length for 0800",
            just_digits,
            just_digits,
            just_digits.length
          );
      ...
      if (new RegExp("^9(00|06|09)").test(just_digits)) {
        if (debug)
          console.log(
            "invalid: incorrect length for 09(00|06|09)",
            just_digits,
            just_digits.length
          );
        return just_digits.length == 7 || just_digits.length == 10;
